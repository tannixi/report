...
class MedCLIPFeatureExtractor(CLIPFeatureExtractor):
    ...
    def __call__(self, 
        ...
        **kwargs) -> BatchFeature:
        ...
        )  #is_batched：定义一个布尔变量 is_batched 来判断输入是否为一个图像的批处理（即列表或元组形式）。如果是，则保持原样；如果不是，将单个图像封装成列表。
...
class MedCLIPProcessor(CLIPProcessor):
    ...
    def __init__(self):
        feature_extractor = MedCLIPFeatureExtractor()  #提取图像特征
        tokenizer = AutoTokenizer.from_pretrained(constants.BERT_TYPE) #文本分词器
        tokenizer.model_max_length = 77 #分词器属性，分词器在处理文本时将使用的最大长度限制为 77 个字符
...
class ImageTextContrastiveDataset(Dataset):
    ...
    def __init__(self, datalist=['mimic-cxr-train', 'chexpert-train'], imgtransform=None) -> None:
        ...
        for data in datalist:
            filename = f'E:/code/MedCLIP-main/local_data/chexpert-train-meta.csv'
        ...
        self.sentence_label = pd.read_csv('E:/code/MedCLIP-main/local_data/sentence-label.csv', index_col=0).fillna(0)
...
class ZeroShotImageDataset(Dataset):
    def __init__(self,
        ...
        df_list = []
        for data in datalist:
            filename = f'E:/code/MedCLIP-main/local_data/chexpert-val-meta.csv'
            print('load data from', filename)
            df = pd.read_csv(filename, index_col=0)
            df_list.append(df)
        self.df = pd.concat(df_list, axis=0).reset_index(drop=True)
