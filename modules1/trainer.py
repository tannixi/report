...
class BaseTrainer(object):
    def __init__(self, model, criterion, metric_ftns, optimizer, args, lr_scheduler):
        ...
        self.metric_ftns = metric_ftns #评价指标
    ...
    def _save_checkpoint(self, epoch, save_best=False):
        ...
        if save_best:
            best_path = os.path.join(self.checkpoint_dir, 'normlo_model.pth')
            torch.save(state, best_path)
            self.logger.info("Saving current best: normlo_model.pth ...")
...
class Trainer(BaseTrainer):
    def __init__(self, model, criterion, metric_ftns, optimizer, args, lr_scheduler, logger, train_dataloader,
        ...
        self.img_con_loss_weight = float(args.weight_img_con_loss)
        self.txt_con_loss_weight = float(args.weight_txt_con_loss)
        self.img_bce_loss_weight = float(args.weight_img_bce_loss)
        self.txt_bce_loss_weight = float(args.weight_txt_bce_loss)
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (images_id, images, reports_ids, reports_masks, labels) in enumerate(self.train_dataloader):
            ...
            loss.backward()
            self.optimizer.step()
            self.args.log_period=int(self.args.log_period)
