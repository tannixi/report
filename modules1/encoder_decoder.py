...
def memory_querying_responding(query, key, value, mask=None, dropout=None, topk=15):
    d_k = query.size(-1)
    scores = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(d_k)
    if mask is not None:
        scores = scores.masked_fill(mask == 0, float('-inf'))
    selected_scores, idx = scores.topk(int(topk))
...
class EncoderDecoder(AttModel):
    ...
    def __init__(self, tokenizer, mode = 'train'):
        super(EncoderDecoder, self).__init__(tokenizer)
        ...
        self.img_margin = 0.4
        self.txt_margin = 0.4
        self.num_protype = int(20)
        # args.num_cluster=int(14)
        # args.num_protype=int(20)
        # args.cmm_dim=int(512)
        # args.d_txt_ebd=int(768)
        # args.d_img_ebd=int(2048)
        ...
        self.txt_dim_reduction = nn.Linear(768, 512)

        self.dim_reduction = nn.Linear(768 + 2048, 512)

        self.img_dim_reduction = nn.Linear(2048, 512)
        ...
        self.cmn = MultiThreadMemory(8, 512, topk=15)

        self.model = self.make_model(tgt_vocab, self.cmn)
        self.logit = nn.Linear(512, tgt_vocab)

        self.protypes = nn.Parameter(torch.FloatTensor(20 * 14, 768 + 2048))
        if mode == 'train':
            init_protypes = torch.load('./files1/iu_xray/init_prototypes.pt').float()
    ...
    def _prepare_feature_forward(self, att_feats, att_masks=None, seq=None, labels=None,):
        ...
        max_num_protype = int(labels.sum(-1).max().item()) * int(self.num_protype)
        protypes = self.dim_reduction(self.protypes)
        query_matrix = protypes.new_zeros(att_feats.size(0), max_num_protype, protypes.shape[-1])
        cmn_masks = protypes.new_zeros(query_matrix.shape[0], att_feats.size(1), max_num_protype)
