...
class BaseCMN(AttModel):
    ...
    def __init__(self, tokenizer):
        super(BaseCMN, self).__init__(tokenizer)
        self.num_layers = 3
        self.d_model = 512
        self.d_ff = 512
        self.num_heads = 8
        ...
        self.cmn = MultiThreadMemory(8, 512, topk=32)

        self.model = self.make_model(tgt_vocab, self.cmn)
        self.logit = nn.Linear(512, tgt_vocab)

        self.memory_matrix = nn.Parameter(torch.FloatTensor(2048, 512))
        nn.init.normal_(self.memory_matrix, 0, 1 / 512)
    ...
    def _save_attns(self, start=False):
        ...
        self.attention_weights.append([layer.src_attn.attn.cpu().detach().numpy() for layer in self.model.decoder.layers])
